...
public class KValid extends RandomizableClusterer implements
    ...
    /** Maximum interations. */
    protected int m_maxInteration = 100;
    ...
    protected int m_validationMethod = ELBOW_METHOD;
    ...
    /** Minimum k. */
    protected int m_minimumK = 2;

    /** Maximum k. */
    protected int m_maximumK = 25;

    /** Cascade. */
    protected boolean m_cascade = true;
...
//        m_silhouetteIdx = new ArrayList<SilhouetteIndex>();
		...
		/* Cascade k-Means. */
        for (int i = m_minimumK; i <= m_maximumK; i++) {
            ...
            if (m_validationMethod == ELBOW_METHOD) {
                m_elbow.add( m_skmeans.getSquaredError());
//                System.out.println("iteration (#clusters) = " + i + " SSE = " + m_elbow.get(i - m_minimumK));
        ...
        m_bestK += m_minimumK;
        m_skmeans = new SimpleKMeans();
        System.out.println("best number of cluster = " + m_bestK);
