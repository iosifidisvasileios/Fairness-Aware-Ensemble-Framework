...
public class SMT extends IteratedSingleClassifierEnhancer
        implements Classifier {
  private final static Logger log = Logger.getLogger(SMT.class.getName());
...
//  protected int m_NumIterations = 20;
  ...
  public SMT(Classifier baseClassifier,
  ...
  public void done() throws Exception {
    ...
    if ((abs(disc) <= bound ))
      return ;
    // calculate the threshold based on training data
...
/*
    ...
    Evaluation fixedEval = new Evaluation(m_TrainingData);
    fixedEval.evaluateModel(this, m_TrainingData);
    double disc = 100*calculateEquallizedOpportunity(this, m_TrainingData);
...
//        log.info(dist[0] + "," + dist[1]);
  ...
  public double[] distributionForInstance(Instance instance) throws Exception {
    ...
    if (eqOp < 0) {
      if (sums[1] >= threshold && !instance.stringValue(protectedValueIndex).equals(protectedValueName)){
        sums[0] = 0;
        sums[1] = 1;
      }
    }else if(eqOp > 0){
